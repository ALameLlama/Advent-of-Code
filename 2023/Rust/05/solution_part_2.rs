...
fn aoc_process(input: &str) -> Result<u64, &'static str> {
    ...
    let locations = seeds.par_chunks(2).flat_map(|chunk| {
        let (seed, range) = (chunk[0], chunk[1]);

        let parsed_locations = (seed..(seed + range)).into_par_iter().map(|current_seed| {
            let soil = get_map_value(current_seed, &seed_to_soil_map);
        ...
        println!("proccessed seed range: {seed}");
        parsed_locations
    });
